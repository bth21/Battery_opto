...
Conversion:
    ...
    clrf    extr_1	    ;clearing all registers for conversion
    clrf    extr_2
    clrf    extr_3
    clrf    high_res_1
    clrf    low_res_1
    ...
    clrf    low_res_24x8_1
    clrf    high_res_24x8_1
    clrf    low_res_24x8_2
    clrf    high_res_24x8_2
    clrf    low_res_24x8_3
    clrf    high_res_24x8_3
    clrf    carry
    ...
    movf    extr_3, W    ;combining low byte of conversion
    swapf   WREG
    movwf   ADRESL       ;moving to ADRESL for LCD display
   
    movf    extr_1, W    ;combining high byte of conversion
    swapf   WREG
    addwf   extr_2, W
...
Multiply_16x16:                     ;obtains the first byte of the hex number in decimal
    ...
    movf    one_low, W
    mulwf   ADRESL               ;multiplying low byte by low byte of multiplication constant
   
    movff   PRODH, high_res_1
    movff   PRODL, low_res_1
   
    movf    one_high, W          ;multiplying high byte by high byte of multiplication constant
    mulwf   ADRESH
   
    movff   PRODH, high_res_cross
    movff   PRODL, low_res_cross
   
    movf    one_low, W           ;multiplying high byte by low byte of multiplication constant
    mulwf   ADRESH
   
    movf    PRODL, W             ;adding the cross terms
    addwf   high_res_1, F
    movf    PRODH, W
    addwfc  low_res_cross, F     ;adding the carry bit to next term
    clrf    WREG
    addwfc  high_res_cross, F
   
    movf    one_high, W          ;multiplying low byte by high byte of multiplication constant
    mulwf   ADRESL
   
    movf    PRODL, W            ;adding next cross terms
    addwf   high_res_1, F
    movf    PRODH, W
    addwfc  low_res_cross, F     ;adding carry bit to next term
    clrf    WREG
    addwfc  high_res_cross, F    ;solution to mult 16x16 stored as: HRC(00) LRC HR1 LR1
   
    return
   
Multiply_24x8_1:                    ;multiplication to obtain second digit in decimal
...
Multiply_24x8_2:                    ;multiplication to obtain third digit in decimal
